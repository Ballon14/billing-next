  ...
  return function (handler) {
    return async (req, ...args) => {
      try {
        ...
        return error('Invalid request body: ' + e.message, 400)
      }
    }
  }
}
...
export const userSchema = z.object({
  ...
  role: z.enum(['SUPER_ADMIN', 'ADMIN', 'TECHNICIAN', 'CUSTOMER']).default('ADMIN'),
  customer_id: z.union([z.number(), z.string()]).transform(v => Number(v)).optional().nullable(),
})

export const userUpdateSchema = z.object({
  name: z.string().min(1, 'Name is required').max(100),
  email: z.string().email('Invalid email'),
  password: z.string().min(6, 'Password min 6 characters').max(100).optional().or(z.literal('')),
  role: z.enum(['SUPER_ADMIN', 'ADMIN', 'TECHNICIAN', 'CUSTOMER']).default('ADMIN'),
  customer_id: z.union([z.number(), z.string()]).transform(v => Number(v)).optional().nullable(),
})
